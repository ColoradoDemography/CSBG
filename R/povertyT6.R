...
POVPlot <- plot_ly(f.saipecty_PL, 
                   x = ~year, 
                   y = ~value, 
                   name = ~age_cat, type = 'scatter', mode = 'lines', text = ~indText, hoverinfo = 'text',
                   transforms = list(
                      list(
                        type = 'filter',
                        target = ~geoname,
 ...
 layout(title=grTitle, yaxis = list(title = 'Percent',tickformat = "%"), xaxis=list(title='Year'),
          showlegend = TRUE,
    updatemenus = list(
      list(
        type = 'dropdown',
...
# Creating Table data file
    ...
    f.saipe_PCT$povpcttot <- f.saipe_PCT$povpcttot * 100
    f.saipe_PCT$povpctlt5 <- f.saipe_PCT$povpctlt5 * 100
    f.saipe_PCT$povpct0517 <- f.saipe_PCT$povpct0517 * 100
    f.saipe_PCT$povpctgt17 <- f.saipe_PCT$povpctgt17 * 100
 ...
 # Flex Table
  ...
  #bind list
  outList <- list("plot"= POVPlot, "data" = f.saipecty_tab, "FlexTable" = f.povFlex)
